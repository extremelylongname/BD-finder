...
def load_models():
    ...
    model_paths = {
        ...
        'otypeclassif': os.path.join(models_dir, 'otypeclassif2.joblib'),
    }
...
def EnsembleClassifier(df):
    ...
    models = list(load_models().values())
    ...
    for i in range(0, len(df)):
        ...
        if otype_pred1:
            if 'W1mag' in df.columns.values:
                ...
                model2 = torch.jit.load(os.path.join(os.path.join(os.path.dirname(__file__), 'ml-models'), 'bd_ann.pt'))
                model2.eval()
